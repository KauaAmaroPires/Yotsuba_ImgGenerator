...
module.exports = async ({ req: req, res: res }) => {
  ...
  const Canvas = canvas.createCanvas(680, 505);
  ...
  const img2 = await canvas.loadImage('./src/image/bolsoquadro.png');
...
const transform = async (image) => {
  ...
  ctx.rotate(4.6 * Math.PI / 180);

  ctx.drawImage(img, 313, 10, 85, 117);
